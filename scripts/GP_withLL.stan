data {
  ...
  int<lower=0> row_index[N_tot];
  int<lower=0> col_index[N_tot];
...
generated quantities {
  ...
  for (i in 1:N_tot)
    log_lik[i] = bernoulli_lpmf(y[row_index[i],col_index[i]] | 1 - (1 - Phi(z[row_index[i]]))^m[row_index[i],col_index[i]]);
}
